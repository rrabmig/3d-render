class Environment {
    ...
    drawPolygon(v1: Vector3, v2: Vector3, v3: Vector3, ctx: CanvasRenderingContext2D, color?: string, i1?: number, i2?: number, i3?: number) {
        color = color? color : Math.floor(Math.random()*16777215).toString(16);
        ctx.strokeStyle = color
        ...
        // рисуем вершины
        this.drawPointByScreenCoords(p1, ctx, '#000000', 5, i1);
        this.drawPointByScreenCoords(p2, ctx, '#000000', 5, i2);
        this.drawPointByScreenCoords(p3, ctx, '#000000', 5, i3);

        ctx.stroke();
    }

    drawObject(primitive: Primitive, ctx: CanvasRenderingContext2D, showIndexes: boolean = false, color?: string, ) {
        color = color? color : Math.floor(Math.random()*16777215).toString(16);
        ...
        for (let i = 0; i < indexes.length; i+=3) {
            ...
            if (showIndexes) {
                this.drawPolygon(v1, v2, v3, ctx, color, i1, i2, i3);
            } else {
    ...
    drawAll(ctx: CanvasRenderingContext2D) {
        ...
        for (const primitive of this.primitives) {
            this.drawObject(primitive, ctx, false);
        }
    }
